...
describe("Airdrop Contract", function () {
  ...
  it("should allow valid address to redeem partial tokens", async function () {
    ...
    const { witnesses } = merkleProof(addresses, { address: addr1.address, amount: totalAmount });
    console.log("witnesses: ", witnesses, "rootHash: ", merkleRootHash);
    console.log("merkleVerification: ", merkleVerification(merkleRootHash , { address: addr1.address, amount: totalAmount }, witnesses));

    // addr1 calls the airdrop function with a partial redemption
    await expect(airdrop.connect(addr1).airdrop(witnesses, totalAmount, redeemAmount))
  ...
  it("should allow multiple partial redemptions", async function () {
    ...
    await expect(airdrop.connect(addr1).airdrop(witnesses, totalAmount, firstRedeem))
      .to.emit(airdrop, "TokensAirdropped")
      .withArgs(addr1.address, firstRedeem);

    // Second partial redemption
    await expect(airdrop.connect(addr1).airdrop(witnesses, totalAmount, secondRedeem))
      .to.emit(airdrop, "TokensAirdropped")
  ...
  it("should fail for invalid proof", async function () {
    ...
    await expect(airdrop.connect(addr2).airdrop(witnesses, totalAmount, redeemAmount))
  ...
  it("Should unpause the contract and allow a valid airdrop claim", async function () {
    ...
    await expect(airdrop.connect(addr1).airdrop(witnesses, totalAmount, ethers.parseEther("50")))
      .to.emit(airdrop, "TokensAirdropped")
